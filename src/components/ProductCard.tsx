...
export default function ProductCard({
  ...
  return (
    <div className="card text-black cursor-pointer card-compact m-3 w-80 bg-gray-300 shadow-xl duration-300 hover:shadow-2xl relative">
      <div
      ...
      <div className="card-body">
        ...
        <div className="card-actions justify-end z-20">
          <button onClick={AddToCart} className="btn  btn-circle btn-ghost ">
            <BsCartPlus className="text-2xl text-gray-600 font-semibold" />
          </button>
          <button
            onClick={AddToBookmark}
            className="btn btn-circle btn-ghost absolute top-0 right-0 "
          >
            <MdFavorite className="text-xl text-red-600 font-semibold" />
          </button>
        </div>
      </div>
    </div>
